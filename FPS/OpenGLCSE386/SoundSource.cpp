...
void SoundSource::initialize()
{
	...
	alSourcef(uiSource, AL_GAIN, 0.5f); // source gain value 
...
void SoundSource::setSoundPosition(vec3 input){
	position = input;
}

void SoundSource::setRelative(bool input){
	if(input)
...
void SoundSource::setPitch(float input){
	alSourcef(uiSource, AL_PITCH, input); // pitch multiplier 
}

void SoundSource::setVolume(float input){	
	alSourcef(uiSource, AL_GAIN, input); // source gain value
}
